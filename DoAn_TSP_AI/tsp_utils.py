...
def get_distance_matrix_string(cities_data, dist_matrix):
    # Tạo chuỗi text hiển thị ma trận (Đã bỏ [:5] để hiển thị tên đầy đủ)
    matrix_string = "Distance Matrix:\n\t"
    matrix_string += "\t".join(city[2] for city in cities_data) + "\n"
    for i, row in enumerate(dist_matrix):
        matrix_string += cities_data[i][2] + "\t" + "\t".join(f"{dist:.1f}" for dist in row) + "\n"
